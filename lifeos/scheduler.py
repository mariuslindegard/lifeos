...
def start_scheduler() -> None:
    ...
    scheduler.add_job(lambda: _run("hourly_analysis"), "interval", hours=1, id="hourly_analysis", replace_existing=True)
    scheduler.add_job(lambda: _run("nightly"), "cron", hour=3, minute=15, id="nightly", replace_existing=True)
    scheduler.start()
